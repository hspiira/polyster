...
**Status: ✅ Done.** Verified live 2026-08-12 with all three tenants created and present in the same Supabase project simultaneously (not sequentially) -- Kampala Bespoke Tailors (`business_type: tailor`, default feature flags), City Rental Wear (`business_type: rental`, `catalogue`/`inventory` on), and NORTH//FOUND (`business_type: apparel_brand`, the full flag set from section 84). A 38-check live test exercised every single ability each tenant's list names -- all 38 passed. Isolation was checked with all three tenants live at once, across six tables (`clients`, `orders`, `products`, `staff`, `garment_units`, `collections`): each tenant's own session saw only rows carrying its own `shop_id`, and a service-role ground-truth query confirmed `garment_units` exists only for the one tenant that actually used that feature.

**Two real, pre-existing gaps this pass found and closed, not routed around:** Tenant B's own acceptance list requires "Take deposit" and "Refund deposit," and neither was actually possible before this pass -- `createOrder` hardcoded `rental_deposit_minor` to `0` with no form field to set it, and `deposit_refunded_at` was displayed in both `OrderDetail.tsx` and `web/Inspector.tsx` but had no write path anywhere to ever set it. Both predate every phase in this document (Phase 0/1 era); they surfaced only now because this is the first time anything in this project exercised the rental deposit flow end to end rather than by reading the code and assuming it worked. Fixed: a "Deposit" field on the order form (rental orders only), threaded through `createOrder`/`updateOrderHeader`, and a new `refundDeposit()` write with a "Refund deposit" button on both shells' order views (gated by the `payments.refund` permission from Phase 12, a natural fit). Fixing this also caught a second, smaller bug it made reachable for the first time: the refunded-at display called `formatDate()` (which expects a plain date and silently returns the raw string unmodified for anything else) on a full timestamp column, printing a raw ISO string instead of a real date; switched to `formatDateTime()` on both shells. All fixed and reverified live -- confirmed in the same 38-check pass and with a dedicated Playwright run against the real form and both order-detail views.
