...
- [ ] **Shop A's account cannot read Shop B's data.** Test with real queries against the tables *and* against `order_balances` specifically -- the view is the one object in this schema that can leak across tenants while every base table looks correctly locked down. `select * from order_balances` as shop A must return only shop A's rows. `pnpm verify:rls` (see README setup step 3a) checks the structural preconditions for this -- RLS enabled and policies present on every table, `security_invoker` on the view, `anon`/`authenticated` not bypassing RLS -- but running it is not a substitute for this line item: it cannot log in as shop A and prove the query actually returns zero rows of shop B's data.
